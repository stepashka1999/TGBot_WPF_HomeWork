...
namespace ConsoleBot
{
    class MyTelegramBot
    {
        ...
        public MyTelegramBot(MainWindow mainWindow,string token)
        {
        ...
        public ObservableCollection<ChatMessage> History = new ObservableCollection<ChatMessage>();
        ...
        /// <summary>
        /// Проверяет существует ли директория для хранения временных файлов и задает ее
        /// </summary>
        private void DefaultDirectory()
        {
            if (File.Exists(config))
            {
                ...
                if (Directory.Exists(defaultDonwloadPath)) return;
                else
                {
                    System.Windows.MessageBox.Show("Директория для хранения временных файлов не задана или не существует.\nЗадайте директорию.", "Error", MessageBoxButton.OK, MessageBoxImage.Information);
                    SetNewDirectory();
                    return;
                }
            }
        ...
        /// <summary>
        /// Записывает новую дирекуторию в файл
        /// </summary>
        private void SetNewDirectory()
        {
            ...
            {
                defaultDonwloadPath = fbd.SelectedPath;
                WriteDirectoryToConfig(defaultDonwloadPath);
            }
        }
        
        /// <summary>
        /// Записывает дефолтную директориб в конфиг
        /// </summary>
        /// <param name="dirPath"></param>
        private void WriteDirectoryToConfig(string dirPath)
        {
            File.Delete(config);
            File.WriteAllText(config, dirPath);
        }

        /// <summary>
        /// Считывает директорию с файла
        /// </summary>
        /// <returns></returns>
        private bool ReadDirectoryFromFile()
        {
            defaultDonwloadPath = File.ReadAllText(config);
            return Directory.Exists(defaultDonwloadPath);
        }
        ...
        private async void HistoryUpdater(object sender, Telegram.Bot.Args.MessageEventArgs e)
        {
            mainWindow.Dispatcher.Invoke(()=>
            {
                History.Add(new ChatMessage(e.Message));
            });
        }
        ...
        /// <summary>
        /// Отпарвляет пользователю сообщение
        /// </summary>
        /// <param name="chatID">ID чата</param>
        ...
        /// </summary>
        /// <returns></returns>
        public async Task SaveHistoryAsJson()
        {
